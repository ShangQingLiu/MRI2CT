...
def train():
    ...
    #TODO: Write into Two Training Module
    ...
    for epoch in range(start_epoch, args.N_epoches + 1):
        ...
        if step % 20000 == 0:     #args.N_step_save
            path = os.path.join(basedir, name, "epoch{}_step{}.pth".format(epoch, step))
            torch.save(
                {
                    "epoch": epoch,
                    "step": step,
                    ...
                    #"latent_embeddings": latent_embeddings.state_dict(),
                },
                path,
            )
...
def forward(self, x, object_latent_embedding):
    object_latent_matrix = object_latent_embedding.view(-1, self.latent_dim, self.matrix_size)
    x = self.embedder(x)
    identity = x
    ...
    if self.mode == 1:  # addition mode
        ...
        for i, linear in enumerate(self.pts_linears):
            ...
            x = linear(x)
            if i in self.skips and (i+1 < self.matrix_size): #added check to ensure we only concatenate for non-skip layers or if within the matrix size
                x = torch.cat([identity, x], -1)
